...
const CustomNode = ({ data }: { data: any }) => {
  ...
  return (
    <div 
      className={`relative px-3 py-3 shadow-md rounded-xl border-2 ${bgColor} ${borderColor} w-[280px] hover:shadow-xl hover:scale-[1.02] transition-all cursor-pointer z-50 bg-white`}
    ...
    connectedMembers.forEach((member) => {
      if (member.parentId && connectedIds.has(member.parentId)) {
        initialEdges.push({
          ...
          sourceHandle: 'source',
          targetHandle: 'target',
          type: 'smoothstep', // Jalur organik bersudut tumpul untuk mengaburkan benturan struktur
          animated: true,
      ...
      if (member.spouseId && connectedIds.has(member.spouseId)) {
        ...
        if (!initialEdges.some(e => e.id === edgeId1 || e.id === edgeId2)) {
          initialEdges.push({
            ...
            type: 'smoothstep', // Garis melengkung organik agar membedakan aliran romansa dari aliran ortogonal darah
